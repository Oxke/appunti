...
\begin{proof}
    ...
    -a} \right)}^{n} d \zeta = \\ &= \sum_{n=0}^{\infty} {\left( \frac{1}{2\pi i}
                \int_{\partial D_\rho{(a)}} \frac{f{(\zeta)}}{{(\zeta-a)}^{n+1}} d
        \zeta\right)} {(z-a)}^{n}
    \end{align*}
...
\begin{theorem}[Sviluppo di Laurent]
Siano \(0 \le r_{1} < r_{2} < +\infty\) e \(a \in \mathbb{C}\).
Consideriamo\footnote{Nel caso di \(r_{1} = 0, r_{2}= r\) allora si indica anche
    \(D^{*}_r{(a)} = D_r{(a)} \sminus \{a\} \)}
\[
...
\end{theorem}
\begin{proof}
    Per assurdo supponiamo che esista un \(w \in \mathbb{C}\) e \(r > 0\)
    tali che
\[
...
\]
    ...
    \(\displaystyle \varphi{(z)} = \frac{1}{f{(z)} - w}\) se \(z\neq a\). Ne segue che
