...
\end{example}
\begin{theorem}[Gruppo fondamentale del prodotto]\label{thm:prod_p1}
...
\end{proof}
Ora l'idea è usare questa elica per definire il ``grado'' di un laccio, ossia il
numero di avvolgimenti.

\begin{definition}{Sollevamento}
    Dato \(f : I \to S^{1}\) continua, un'applicazione continua \(\tilde{f} : I
    \to \mathbb{R}\) tale che \(e \circ \tilde{f}= f\) si chiama
    \textbf{sollevamento} di \(f\)  rispetto a \(e\).

    Inoltre se \(x_{0} = f{(0)}\) e fisso \(\tilde{x_{0}} \in \mathbb{R}\) tale
    che \(e{(\tilde{x_{0}})} = x_{0}\). Allora se \(\tilde{f}{(0)} =
    \tilde{x_{0}}\), \(\tilde{f}\) si chiama sollevamento con punto base
    \(\tilde{x_{0}}\).
\end{definition}

\begin{remark}
    Se \(\tilde{f}\) è un sollevamento di \(f\) allora \(\tilde{f}{(0)} \in
    e^{-1}{(f{(0)})}\) 
\end{remark}

\begin{theorem}[Esistenza di sollevamenti di cammini]
    Data \(f : I \to S^{1}\) un cammino, sia \(x_{0} = f{(0)}\). Allora per ogni
    \(\tilde{x_{0}} \in e^{-1}{(\tilde{x_{0}})}\), esiste unico un sollevamento
    di \(f\) rispetto a \(e\) con punto base \(\tilde{x_{0}}\).
\end{theorem}
\begin{proof}
    Sia \(f: I \to S^{1}\). 
