...
\section{Bimoduli e omomorfismi}
...
\end{tikzcd} \]
        commuta. Questo è vero perché, per ogni \(f \in \mathrm{Hom}_A{(A, M)}\),
    
        \[
            g{(\alpha_M{(f)})} = g{(f{(1)})}
            \qquad\text{e}\qquad
            \alpha_N{(g_*{(f)})} = {(g \circ f)}{(1)}
        \]
\end{example}
...
\begin{example}{}
    ...
    \in \mathrm{Hom}_B{(M, N)}\), \({(af)}{(x)} = f{(ax)} = {(fa)}{(x)}\).
...
\begin{minipage}{0.48\textwidth}
    \begin{align*}
        M\otimes_A -: A\mathtt{-Mod} &\longrightarrow \text{Ab} \\
...
\begin{minipage}{0.48\textwidth}
    \begin{align*}
        - \otimes_A N: \mathtt{Mod-}A &\longrightarrow \text{Ab} \\
...
\begin{definition}{Funtore aggiunto}
    ...
    \mathcal{C}\) funtori. Si dice che \(F\) è \emph{aggiunto sinistro} di
...
\begin{eser}{}
    ...
    Il suggerimento è molto potente, e ci suggerisce in realtà che
    se \(\mathrm{Zero}\) è il funtore suggerito, allora
    \[
      \mathrm{Zero} \dashv \mathrm{Ker}
    \]
    e dunque \(\mathrm{Ker}\) è aggiunto destro tra categorie abeliane, e dunque
    esatto a sinistra. Siano allora \(X \in \mathcal{A}\) e \(f : Y \to Y' \in
    \mathtt{Mor}{(\mathcal{A})}\) 
    \begin{align*}
        \varphi_{\mathrm{Zero}{(X)}, Y} :
        \mathtt{Mor}{(\mathcal{A})}{(X \mapsto 0, f)} &\longrightarrow
        \mathcal{A}{(X, \mathrm{Ker}f)} \\
         {(g: X \to Y, g' : 0 \to Y')} &\longmapsto \varphi_{\mathrm{Zero}{(X)},
         Y} (\alpha) = {(\exists !)}\,h \text{ tale che } g = \mathrm{ker}f \circ h
    \end{align*}
    Dove per definire \(h\) si è usata la proprietà universale del
    \(\mathrm{Ker}\), che vale perché \(f \circ g = g' \circ 0 = 0\). Notare che
    è possibile definire l'inversa di \(\varphi_{\mathrm{Zero}{(X)}, Y} \) come
    \(g = \mathrm{ker}f \circ h, g' : 0 \to Y'\) poiché \(g'\) esiste unica
    per le proprietà di 0. Allora si ha che \({(g, g')}\) è una trasformazione
    naturale di \(X \mapsto 0\) in \(f : Y \to Y'\) perché \(f \circ g = f \circ
    \mathrm{ker}f \circ h = 0 \circ h = 0 = g' \circ 0\).

    È facile ma contoso mostrare che \(\varphi \) è naturale.

    Per il \(\mathrm{Coker}\) il discorso è analogo e duale.
\end{eser}
...
N\) è \(A\)-bilanciata universale.
    \item le due funzioni tra \(\mathrm{Bal}_A\) e \(\mathrm{Hom}_A\) sono
        ...
        \item
        \[
            \mathrm{Bal}_A^{B}{(M \times N, P)} =
            \{f \in \mathrm{Bal}_A{(M \times N, P)} \mid
            f \text{ è \(B\)-lineare nel primo argomento}\}
        \]
\end{itemize}
...
\begin{corollary}{}\label{cor:mod_tens}
    ...
    \[
        M \otimes_A - \dashv \mathrm{Hom}_A{(M, -)}
    \]
    come funtori \(A\mathtt{-Mod} \to A\mathtt{-Mod}\).
\end{corollary}
...
\section{Proiettivi, iniettivi e piatti}
...
\begin{enumerate}[label = \arabic*.]
    \item Basta dimostrare il caso \(X = \coprod_{\lambda \in \Lambda}
        ...
        \[
          \mathcal{A}{(X, Y)} = \mathcal{A}{\left( \coprod_{\lambda \in \Lambda}
...
\begin{proof}{}
    ...
    \rightarrowtail M\) mono in \(A\mathtt{-Mod}\) con \(M\) iniettivo.
