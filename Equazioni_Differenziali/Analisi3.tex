...
\end{figure}
    ...
    \[
        x(t) =
        \sqrt{c_{1}^2+c_{2}^2}\left(\frac{c_{1}}{\sqrt{c_{1}^2+c_{2}^2}}\cos(\omega
        t) + \frac{c_{2}}{\sqrt{c_{1}^2+c_{2}^2}}\sin(\omega t)\right)
    \]
...
Analogamente equazioni di ordine superiore possono essere ricondotte a sistemi
...
\begin{proof}
    ...
    di \((P)\). Consideriamo i valori di \(t \in J \cap [t_{0}, t_{0} +
    \alpha]\) e analogamente per \(t \le  t0\) e vogliamo mostrare che \(z(t) =
    x(t)\) con \(x(\cdot )\) la soluzione costruita prima. Allora abbiamo per
    ipotesi che
    \[
